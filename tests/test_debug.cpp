...
TEST_F(DebugTest, DirectLambdaCall) {
    json script = json::array({
        "let",
        json::array({
            json::array({"f", json::array({"lambda", json::array({"y"}), json::array({"$", "/x"})})}),
            json::array({"x", 42})
        }),
...
TEST_F(DebugTest, LambdaWithParameters) {
    json script = json::array({
        "let",
        json::array({
            json::array({"add", json::array({"lambda", json::array({"x"}), json::array({"+", json::array({"$", "/x/0"}), json::array({"$", "/x/1"})})})})
        }),
...
TEST_F(DebugTest, LambdaVariableInFilter) {
    json script = json::array({
        "let",
        json::array({
            json::array({"is_even", json::array({"lambda", json::array({"x"}), json::array({"==", json::array({"%", json::array({"$", "/x"}), 2}), 0})})})
        }),
...
TEST_F(DebugTest, LambdaWithComplexBody) {
    json script = json::array({
        "let",
        json::array({
            json::array({"complex", json::array({"lambda", json::array({"x"}), json::array({
                "let",
                json::array({
                    json::array({"a", json::array({"$", "/x/0"})}),
                    json::array({"b", json::array({"$", "/x/1"})})
                }),
                json::array({"+", json::array({"*", json::array({"$", "/a"}), 2}), json::array({"$", "/b"})})
            })})})
        }),
...
TEST_F(DebugTest, LambdaParameterAccess) {
    json script = json::array({
        "let",
        json::array({
            json::array({"access", json::array({"lambda", json::array({"x"}), json::array({"$", "/x/2"})})})
        }),
...
TEST_F(DebugTest, LambdaErrorHandling) {
    ...
    json script = json::array({
        "lambda", json::array({"x", "y"}), json::array({"$", "/x"})
    });
...
TEST_F(DebugTest, IfOperatorWithLambda) {
    json script = json::array({
        "let", 
        json::array({
            json::array({"test_func", json::array({"lambda", json::array({"x"}), json::array({">", json::array({"$", "/x/0"}), 5})})})
        }), 
        json::array({
            "if", 
