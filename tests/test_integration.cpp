...
TEST_F(IntegrationTest, SimpleDataTransformation) {
    json input_data = json::object({ { "users", json::array({ json::object({ { "id", 1 }, { "name", "Alice" }, { "age", 25 } }), json::object({ { "id", 2 }, { "name", "Bob" }, { "age", 30 } }) }) } });
...
TEST_F(IntegrationTest, MathematicalComputation) {
    json script = json::array({ "let",
        json::array({ json::array({ "numbers", json::object({ { "array", json::array({ 1, 2, 3, 4, 5 }) } }) }) }),
        json::array({ "let",
            json::array({ json::array({ "squared", json::array({ "map", json::array({ "$", "/numbers" }), json::array({ "lambda", json::array({ "x" }), json::array({ "*", json::array({ "$", "/x" }), json::array({ "$", "/x" }) }) }) }) }) }),
            json::array({ "reduce",
                json::array({ "$", "/squared" }),
...
TEST_F(IntegrationTest, StringProcessing) {
    json input_data = json::object({ { "texts", json::array({ "Hello", "World" }) } });

    json script = json::array({ "let",
        json::array({ json::array({ "data", json::array({ "$input" }) }) }),
        json::array({ "let",
            json::array({ json::array({ "texts", json::array({ "$", "/data/texts" }) }) }),
            json::array({ "strConcat",
                json::array({ "$", "/texts/0" }),
                " ",
                json::array({ "$", "/texts/1" }) }) }) });
