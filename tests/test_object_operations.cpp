...
TEST_F(ObjectOperationsTest, ValuesOperatorWithComplexValues) {
    json script = json::array({"values", json{
    ...
    for (const auto& value : values) {
        if (value == json::array({1, 2, 3})) found_data = true;
        else if (value == json{{"version", "1.0"}}) found_meta = true;
        else if (value == true) found_flag = true;
    }
...
TEST_F(ObjectOperationsTest, ObjFromPairsOperator) {
    json script = json::array({"objFromPairs", json::object({
        {"array", json::array({
            json::array({"a", 1}),
            json::array({"b", 2}),
            json::array({"c", 3})
...
TEST_F(ObjectOperationsTest, ObjFromPairsOperatorInvalidPair) {
    json script = json::array({"objFromPairs", json::object({
        {"array", json::array({json::array({"a", 1, 2})})}
    })});
    EXPECT_THROW(exec(script), computo::InvalidArgumentException);
}
...
TEST_F(ObjectOperationsTest, ObjFromPairsOperatorNonStringKey) {
    json script = json::array({"objFromPairs", json::object({
        {"array", json::array({json::array({1, "value"})})}
    })});
    EXPECT_THROW(exec(script), computo::InvalidArgumentException);
}

TEST_F(ObjectOperationsTest, ObjFromPairsOperatorWithZip) {
    json script = json::array({"objFromPairs", json::array({
        "zip",
        json::object({{"array", json::array({"name", "age", "city"})}}),
        json::object({{"array", json::array({"Alice", 30, "NYC"})}})
    })});
...
TEST_F(ObjectOperationsTest, ComplexObjectTransformation) {
    json script = json::array({
        "let",
        json::array({
            json::array({"user", json{{"name", "Alice"}, {"age", 30}, {"email", "alice@example.com"}, {"password", "secret"}}})
        }),
        json::array({
            "objFromPairs",
            json::array({
...
TEST_F(ObjectOperationsTest, DynamicObjectConstruction) {
    json script = json::array({
        "let",
        json::array({
            json::array({"data", json{{"items", json::object({{"array", json::array({"a", "b", "c"})}})}, {"prefix", "item_"}}})
        }),
        json::array({
            "objFromPairs",
            json::array({
