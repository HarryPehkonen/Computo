...
TEST_F(SugarWriterTest, NumberLiteral) { EXPECT_EQ(write("42"), "42"); }

TEST_F(SugarWriterTest, FloatLiteral) { EXPECT_EQ(write("3.14"), "3.14"); }

TEST_F(SugarWriterTest, StringLiteral) { EXPECT_EQ(write(R"("hello")"), "\"hello\""); }

TEST_F(SugarWriterTest, TrueLiteral) { EXPECT_EQ(write("true"), "true"); }

TEST_F(SugarWriterTest, FalseLiteral) { EXPECT_EQ(write("false"), "false"); }

TEST_F(SugarWriterTest, NullLiteral) { EXPECT_EQ(write("null"), "null"); }

// --- Variable access ---

TEST_F(SugarWriterTest, SimpleVariable) { EXPECT_EQ(write(R"(["$", "/x"])"), "x"); }

TEST_F(SugarWriterTest, NestedVariablePath) {
    EXPECT_EQ(write(R"(["$", "/user/name"])"), "user/name");
}

TEST_F(SugarWriterTest, DeepVariablePath) { EXPECT_EQ(write(R"(["$", "/a/b/c/d"])"), "a/b/c/d"); }

// --- Input access ---

TEST_F(SugarWriterTest, InputNoPath) { EXPECT_EQ(write(R"(["$input"])"), "$input"); }
...
TEST_F(SugarWriterTest, Addition) { EXPECT_EQ(write(R"(["+", 1, 2])"), "1 + 2"); }

TEST_F(SugarWriterTest, Subtraction) { EXPECT_EQ(write(R"(["-", 10, 3])"), "10 - 3"); }

TEST_F(SugarWriterTest, Multiplication) { EXPECT_EQ(write(R"(["*", 2, 3])"), "2 * 3"); }

TEST_F(SugarWriterTest, Division) { EXPECT_EQ(write(R"(["/", 10, 2])"), "10 / 2"); }

TEST_F(SugarWriterTest, Modulo) { EXPECT_EQ(write(R"(["%", 10, 3])"), "10 % 3"); }

TEST_F(SugarWriterTest, GreaterThan) { EXPECT_EQ(write(R"([">", 5, 3])"), "5 > 3"); }

TEST_F(SugarWriterTest, LessEqual) { EXPECT_EQ(write(R"(["<=", 1, 2])"), "1 <= 2"); }

TEST_F(SugarWriterTest, EqualEqual) { EXPECT_EQ(write(R"(["==", 1, 1])"), "1 == 1"); }

TEST_F(SugarWriterTest, NotEqual) { EXPECT_EQ(write(R"(["!=", 1, 2])"), "1 != 2"); }

TEST_F(SugarWriterTest, LogicalAnd) {
    EXPECT_EQ(write(R"(["and", true, false])"), "true and false");
}

TEST_F(SugarWriterTest, LogicalOr) { EXPECT_EQ(write(R"(["or", true, false])"), "true or false"); }

// --- Variadic infix ---

TEST_F(SugarWriterTest, VariadicAddition) { EXPECT_EQ(write(R"(["+", 1, 2, 3])"), "1 + 2 + 3"); }

TEST_F(SugarWriterTest, ChainedComparison) { EXPECT_EQ(write(R"([">", 5, 3, 1])"), "5 > 3 > 1"); }

// --- Precedence / parenthesization ---

TEST_F(SugarWriterTest, MulInsideAdd) { EXPECT_EQ(write(R"(["+", ["*", 2, 3], 4])"), "2 * 3 + 4"); }
...
TEST_F(SugarWriterTest, UnaryMinus) { EXPECT_EQ(write(R"(["-", 5])"), "-5"); }

TEST_F(SugarWriterTest, NotOperator) { EXPECT_EQ(write(R"(["not", true])"), "not true"); }

// --- Lambda ---

TEST_F(SugarWriterTest, SimpleLambda) {
    EXPECT_EQ(write(R"(["lambda", ["x"], ["+", ["$", "/x"], 1]])"), "(x) => x + 1");
}
...
TEST_F(SugarWriterTest, ZeroParamLambda) { EXPECT_EQ(write(R"(["lambda", [], 42])"), "() => 42"); }
...
TEST_F(SugarWriterTest, SimpleIf) {
    EXPECT_EQ(write(R"(["if", true, 1, 0])"), "if true then 1 else 0");
}
...
TEST_F(SugarWriterTest, NoArgFunction) { EXPECT_EQ(write(R"(["someOp"])"), "someOp()"); }

// --- Array literals ---

TEST_F(SugarWriterTest, EmptyArrayLiteral) { EXPECT_EQ(write(R"({"array": []})"), "[]"); }

TEST_F(SugarWriterTest, ArrayLiteral) { EXPECT_EQ(write(R"({"array": [1, 2, 3]})"), "[1, 2, 3]"); }
...
TEST_F(SugarWriterTest, EmptyObject) { EXPECT_EQ(write("{}"), "{}"); }
...
TEST_F(SugarWriterTest, NestedFunctionCalls) {
    EXPECT_EQ(
        write(R"(["count", ["filter", ["$", "/arr"], ["lambda", ["x"], [">", ["$", "/x"], 0]]]])"),
