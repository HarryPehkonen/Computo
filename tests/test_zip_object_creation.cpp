...
TEST_F(ZipObjectCreationTest, ZipToObjectWithLetBindings) {
    ...
    json script = json::array({ "let",
        json::array({ json::array({ "keys", json::object({ { "array", json::array({ "x", "y", "z" }) } }) }),
            json::array({ "values", json::object({ { "array", json::array({ 10, 20, 30 }) } }) }) }),
        json::array({ "reduce",
            json::array({ "zip", json::array({ "$", "/keys" }), json::array({ "$", "/values" }) }),
            json::array({ "lambda", json::array({ "x" }),
                json::array({ "let",
                    json::array({ json::array({ "acc", json::array({ "$", "/x/0" }) }),
                        json::array({ "pair", json::array({ "$", "/x/1" }) }) }),
                    json::array({ "merge",
                        json::array({ "$", "/acc" }),
                        json::array({ "obj", json::array({ json::array({ "$", "/pair/0" }), json::array({ "$", "/pair/1" }) }) }) }) }) }),
            json::object() }) });
...
TEST_F(ZipObjectCreationTest, ZipToObjectWithVariablePointerSyntax) {
    ...
    json script = json::array({ "let",
        json::array({ json::array({ "pairs", json::array({ "zip", json::object({ { "array", json::array({ "foo", "bar" }) } }), json::object({ { "array", json::array({ "hello", "world" }) } }) }) }) }),
        json::array({ "reduce",
            json::array({ "$", "/pairs" }),
            json::array({ "lambda", json::array({ "x" }),
                json::array({ "let",
...
TEST_F(ZipObjectCreationTest, ZipToObjectWithDynamicKeys) {
    ...
    json script = json::array({ "let",
        json::array({ json::array({ "prefix", "key_" }),
...
TEST_F(ZipObjectCreationTest, DirectArrayAccessInReduce) {
    ...
    json script = json::array({ "let",
        json::array({ json::array({ "zipped", json::array({ "zip", json::object({ { "array", json::array({ "one", "two" }) } }), json::object({ { "array", json::array({ 1.0, 2.0 }) } }) }) }) }),
        json::array({ "reduce",
            json::array({ "$", "/zipped" }),
            json::array({ "lambda", json::array({ "acc_pair" }),
                json::array({ "merge",
                    json::array({ "$", "/acc_pair/0" }),
                    json::array({ "obj", json::array({ json::array({ "$", "/acc_pair/1/0" }), json::array({ "$", "/acc_pair/1/1" }) }) }) }) }),
            json::object() }) });
