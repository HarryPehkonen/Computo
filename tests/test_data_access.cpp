...
TEST(DataAccessOperators, InputOperatorBasic) {
    ...
    EXPECT_EQ(
        computo::execute(jsom::parse_document(R"(["$input"])"), std::vector<json>{input_data}),
        input_data);

    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$input"])"), std::vector<json>{123}),
              123); // NOLINT(readability-magic-numbers)
    EXPECT_EQ(
        computo::execute(jsom::parse_document(R"(["$input"])"), std::vector<json>{json("hello")}),
        "hello");
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$input"])"),
                               std::vector<json>{json(std::vector<json>{1, 2, 3})}),
              json(std::vector<json>{1, 2, 3}));
}

TEST(DataAccessOperators, InputOperatorErrors) {
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["$input", "extra_arg"])"),
...
TEST(DataAccessOperators, InputsOperatorBasic) {
    ...
    EXPECT_EQ(
        computo::execute(jsom::parse_document(R"(["$inputs"])"), std::vector<json>{input_data}),
        expected_single);
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs"])"), multiple_inputs),
              expected_multiple);
}
...
TEST(DataAccessOperators, InputsOperatorEmpty) {
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs"])"), empty_inputs),
              expected_empty);
}

TEST(DataAccessOperators, InputsOperatorErrors) {
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["$inputs", "extra_arg"])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException);
}
...
TEST(DataAccessOperators, InputsOperatorJSONPointerBasic) {
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/0"])"), inputs),
              json("first"));

    // Access second input: /1
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/1"])"), inputs),
              json("second"));

    // Access third input: /2
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/2"])"), inputs),
              json("third"));
}
...
TEST(DataAccessOperators, InputsOperatorJSONPointerDeepPath) {
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/0/users/1/age"])"), inputs),
              json(25));
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/1/users/1/age"])"), inputs),
              json(28));
}
...
TEST(DataAccessOperators, InputsOperatorJSONPointerSingleInput) {
    ...
    EXPECT_EQ(
        computo::execute(jsom::parse_document(R"(["$inputs", "/0/data/value"])"), single_input),
        json(42));
}

TEST(DataAccessOperators, InputsOperatorJSONPointerArrayElements) {
    // Test accessing array elements within inputs
    std::vector<json> inputs = {jsom::parse_document(R"([10, 20, 30])"),
                                jsom::parse_document(R"({"items": [100, 200, 300]})")};
    ...
    EXPECT_EQ(computo::execute(jsom::parse_document(R"(["$inputs", "/1/items/2"])"), inputs),
              json(300));
}

TEST(DataAccessOperators, VariableOperatorBasic) {
    json script = jsom::parse_document(
        R"(["let", [["x", 42]], ["$", "/x"]])"); // NOLINT(readability-magic-numbers)
...
TEST(DataAccessOperators, VariableOperatorNotFound) {
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["$", "nonexistent_var"])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException);
}
...
TEST(DataAccessOperators, VariableOperatorErrors) {
    EXPECT_THROW(
        computo::execute(jsom::parse_document(R"(["$"])"), std::vector<json>{json(nullptr)}),
        computo::InvalidArgumentException);
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["$", "x", "y"])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException);
    EXPECT_THROW(
        computo::execute(jsom::parse_document(R"(["$", 123])"), std::vector<json>{json(nullptr)}),
...
TEST(DataAccessOperators, LetOperatorBasic) {
    ...
    script
        = jsom::parse_document(R"(["let", [["a", 1], ["b", 2]], ["*", ["$", "/a"], ["$", "/b"]]])");
...
TEST(DataAccessOperators, LetOperatorErrors) {
    EXPECT_THROW(
        computo::execute(jsom::parse_document(R"(["let"])"), std::vector<json>{json(nullptr)}),
        computo::InvalidArgumentException);
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["let", ["x", 10]])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException); // NOLINT(readability-magic-numbers)
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["let", "not_an_array", ["$", "x"]])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException);
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["let", [["x"]], ["$", "x"]])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException);
    EXPECT_THROW(computo::execute(jsom::parse_document(R"(["let", [[10, "x"]], ["$", "x"]])"),
                                  std::vector<json>{json(nullptr)}),
                 computo::InvalidArgumentException); // NOLINT(readability-magic-numbers)
}
...
TEST(DataAccessOperators, VariableSuggestionMultipleOptions) {
    try {
        computo::execute(
            jsom::parse_document(
                R"(["let", [["name", "Alice"], ["num", 42], ["age", 30]], ["$", "/nam"]])"),
            std::vector<json>{json(nullptr)});
        FAIL() << "Expected InvalidArgumentException";
    } catch (const computo::InvalidArgumentException& e) {
        std::string error_msg(e.what());
...
TEST(DataAccessOperators, VariableSuggestionNoCloseMatch) {
    try {
        computo::execute(jsom::parse_document(
                             R"(["let", [["x", 1], ["y", 2]], ["$", "/completely_different"]])"),
                         std::vector<json>{json(nullptr)});
        FAIL() << "Expected InvalidArgumentException";
    } catch (const computo::InvalidArgumentException& e) {
        std::string error_msg(e.what());
...
TEST(DataAccessOperators, VariableSuggestionEmptyScope) {
    try {
        computo::execute(jsom::parse_document(R"(["$", "/nonexistent"])"),
                         std::vector<json>{json(nullptr)});
        FAIL() << "Expected InvalidArgumentException";
    } catch (const computo::InvalidArgumentException& e) {
        std::string error_msg(e.what());
