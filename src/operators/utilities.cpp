...
nlohmann::json str_concat(const nlohmann::json& args, ExecutionContext& ctx) {
    if (args.size() < 2) throw InvalidArgumentException("strConcat requires at least 2 arguments");
...
nlohmann::json merge_op(const nlohmann::json& args, ExecutionContext& ctx) {
    if (args.size() < 2) throw InvalidArgumentException("merge requires at least 2 arguments");
...
nlohmann::json approx_op(const nlohmann::json& args, ExecutionContext& ctx) {
    if (args.size() != 2) throw InvalidArgumentException("approx requires exactly 2 arguments");
