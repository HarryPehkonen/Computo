...
auto car_op(const nlohmann::json& args, ExecutionContext& ctx) -> nlohmann::json {
    if (args.size() != 1) {
        throw InvalidArgumentException("car requires exactly 1 argument");
}
    auto array_val = evaluate(args[0], ctx);
    if (array_val.is_object() && array_val.contains("array")) {
        array_val = array_val["array"];
}
    if (!array_val.is_array() || array_val.empty()) {
        throw InvalidArgumentException("car requires non-empty array");
}
    return array_val[0];
}
...
auto cdr_op(const nlohmann::json& args, ExecutionContext& ctx) -> nlohmann::json {
    if (args.size() != 1) {
        throw InvalidArgumentException("cdr requires exactly 1 argument");
}
    auto array_val = evaluate(args[0], ctx);
    if (array_val.is_object() && array_val.contains("array")) {
        array_val = array_val["array"];
    ...
    if (!array_val.is_array() || array_val.empty()) {
        throw InvalidArgumentException("cdr requires non-empty array");
}
...
auto cons_op(const nlohmann::json& args, ExecutionContext& ctx) -> nlohmann::json {
    if (args.size() != 2) {
        throw InvalidArgumentException("cons requires exactly 2 arguments");
}
    ...
    if (!array_val.is_array()) {
        throw InvalidArgumentException("cons second argument must be array");
}
    nlohmann::json result = nlohmann::json::array();
...
auto append_op(const nlohmann::json& args, ExecutionContext& ctx) -> nlohmann::json {
    if (args.size() != 2) {
        throw InvalidArgumentException("append requires exactly 2 arguments");
}
    auto arr1 = evaluate(args[0], ctx);
    auto arr2 = evaluate(args[1], ctx);
    if (arr1.is_object() && arr1.contains("array")) {
        arr1 = arr1["array"];
}
    if (arr2.is_object() && arr2.contains("array")) {
        arr2 = arr2["array"];
}
    if (!arr1.is_array() || !arr2.is_array()) {
        throw InvalidArgumentException("append arguments must be arrays");
}
    nlohmann::json result = nlohmann::json::array();
...
auto lambda_operator(const nlohmann::json& args, [[maybe_unused]] ExecutionContext& ctx) -> nlohmann::json {
    if (args.size() != 2) {
